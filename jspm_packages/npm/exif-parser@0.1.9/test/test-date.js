...
module.exports = {
  ...
  'test parse given date without timezone': function(test) {
    ...
    var timestamp = date.parseDateWithSpecFormat(dateStr);
    var expectedTimestamp = (15 * years) + (5 * leapYears) + ((31 + 13) * days) + (14 * hours) + (30 * minutes) + 14;
  ...
  'test parse given date with timezone': function(test) {
    ...
    var expectedTimestamp = (yearsFromEpoch - leapYearsCount) * years + leapYearsCount * leapYears + dayCount * days + 23 * hours + 39 * minutes + 6 + 8 * hours;
    test.strictEqual(timestamp, expectedTimestamp);
    test.done();
  },
